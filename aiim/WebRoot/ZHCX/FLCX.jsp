...
/////////卷内事件处理:
	...
	function showArchivesToUseList(archivesUseWay){
		...
		window.showModalDialog("/aiim/DALY/archivesUseAction_addArchivesToUseList.action?archivesTypeIdAndNBXHs="+archivesTypeIdAndNBXHs+"&archivesUseWay="+archivesUseWay,"newwindow","dialogWidth=700px;dialogHeight=480px;center=yes;scroll=no;status=no;resizable=no;help=no;location=no");
		return false;
	}

	//打开查阅申请
	function viewUseList(){
		window.showModalDialog("/aiim/DALY/archivesUseAction_addArchivesToUseList.action?archivesTypeIdAndNBXHs="+"&archivesUseWay="+2,"newwindow","dialogWidth=700px;dialogHeight=480px;center=yes;scroll=no;status=no;resizable=no;help=no;location=no");
	}

	function savedCallBack(){
		var oldActionUrl = $("#tableForm").attr("action");
		$("#tableForm").attr("action","DAGL/archivesInfoManageAction_savedCallBack.action");
		$("#tableForm").ajaxSubmit({
			beforeSubmit:function(){
			var checkBoxs = $("input[name='NBXHS']:checked");
		...
		$("#tableForm").attr("action",oldActionUrl);
    }
	</script>
     ...
     <%--<input type="hidden" name="preSelectRow" id="preSelectRow" /> --%>
        ...
        <table width="100%" style="margin:0px;" align="center" cellpadding="0" cellspacing="0">
		     <tr>
			  <td>   
			     <table width="100%" cellpadding="0" cellspacing="0" border="0">
			       <tr>
				       <td>
                          <input type="image" id="search" src="images/find.gif" alt="隐藏查询(Q)" onmouseover="changeImage(this,'find2.gif')" onmouseout="changeImage(this,'find.gif')" onclick="showfind(this)"/>
					      <input type="image" src="images/advfind.gif" onmouseover="changeImage(this,'advfind2.gif')" onmouseout="changeImage(this,'advfind.gif')" onclick="showADFind()"/>
                          ...
                          <input type="image" src="images/savedCallBack.gif" onmouseover="changeImage(this,'savedCallBack1.gif')" onmouseout="changeImage(this,'savedCallBack.gif')" onclick="savedCallBack()"/>		 
					   ...
					   <td>
						   <div id="location" style="margin-right: 2px; width: 280px; display: inline-block; float: right; margin-top: 0px; color: blue; text-align: right;">
							  <font style="font-size: 12px;"><b>当前位置：</b>综合查询&nbsp;&gt;&gt;&nbsp;&nbsp;分类查询</font>
						   </div>
			...
			<tr>			  
			  ...
			  <s:else>
			  	<td align="center" id="find" style="display:none;">
			  </s:else>
			     <fieldset>
			       ...
			       <input type="hidden" name="advanceQueryCondition" id="advanceQueryCondition" value="${requestScope.advanceQueryCondition }">
			       <table style="font-size: 12px;">
			         ${requestScope.htmlCode }
			       	 <tr>
			           <td></td>
			           <td>
			             <input type="submit" value="" class="submitButton" onclick="clearAdvanceInput()" onmouseover="changeSubmitBgImage(this,'search2.gif')" onmouseout="changeSubmitBgImage(this,'search.gif')"/>
			             <input type="button" value="" class="hideButton" onclick="document.getElementById('find').style.display='none';" onmouseover="changeSubmitBgImage(this,'hide2.gif')" onmouseout="changeSubmitBgImage(this,'hide.gif')" />
			           </td>
			         </tr>
			       </table>
			       </form>
						   	    ...
						   	    if(archivesInfos != null){
						   	      for(int i =0;i<archivesInfos.size();i++){
						   	    	...
						   	    	<input type="hidden" id="typeId<%=archivesInfo.getNBXH() %>" value="<%=archivesInfo.getArchivesTypeID()%>">
						   	    	 ...
						   	    	 <input type="hidden" name="archivesTypeId" id="archivesTypeId" value="${requestScope.archivesTypeId }"></td>
									<td ><%=i+1%></td>
									<%
						   	    	 for(Object dataItem : dataItems.values()){

...
<html>
	...
	function query(){
		...
		targetForm = document.forms["showForm"];
		 targetForm.action="GLYWH/findUserByRealName.action";
		 targetForm.submit();
	}
...
//单击check框事件，控制按钮的可用/不可用状态
	function oneSelect(obj) {
		...
		for (i=0; i<elements.length; i++) {
			if (elements[i].type=="checkbox"  && elements[i].name == "userIDs") {
				iCount++;
...
//打开新增对话框
	function showAdd() {
		var returnValue = window.showModalDialog(				
			"<%=basePath%>GLYWH/userItem.jsp",
			"newwindows",
			"dialogWidth=500px;dialogHeight:260px;top=0;left=0;status:no; toolbar=no;menubar=no;scroll:0;resizable=no;");
		//添加成功刷新窗口
		if(returnValue == 1 ){
	...
	function showItem(rowObj) {		
		var userID = rowObj.id;	
		var returnValue = window.showModalDialog(
				"GLYWH/findUserInfoByUserID.action?userID="+userID,
				"",
				"dialogWidth=500px;dialogHeight:260px;top=0;left=0;status:no; toolbar=no;menubar=no;scroll:0;resizable=no;");
    }
	 ...
	 function clickBatchDel(){
		 targetForm = document.forms["showForm"];
		 targetForm.action="GLYWH/deleteUserInfos.action";
		 targetForm.submit();		 
	}
...
.selectRowColor {
	...
	<body style="overflow: scroll;" >
			...
			<tr>
				<td> 
					...
					<input type="image" src="images/find.gif"
						...
						onmouseout="changeImage(this,'find.gif')" onclick="showFind()" />					
				</td>
				<td>
					<div id="location"
		...
		<div id="findDiv" style="display: none; width: 100%; margin-top: 10px;" align="center">	
			<table class="findTB">			
				<tr style="height: 26px;" align="left">
					...
					<td>
						<input type="text" name="queryRealName" id="queryRealName" style="width: 310px;" />
					</td>
				</tr>
				...
				<tr style="height: 40px;">
					<td></td>
					<td align="left">
						<input type="button" value="查询" style="margin-left: 125px;"
							onclick="query()" />
		...
		<table id="showTable" width="100%"
			...
			<s:iterator value="#request.userInfos" id="id" status="rows">
				<s:if test="#rows.odd==true">
					<tr bgcolor="#eef5ff" id="<s:property value="userID"/>"  onclick="clickRow(this)" ondblclick="showItem(this)">
				</s:if>
				<s:else>
					<tr bgcolor="#e0edff"  id="<s:property value="userID"/>" onclick="clickRow(this)" ondblclick="showItem(this)">
				</s:else>	
					<td><input type="checkbox" name="userIDs" onclick="oneSelect(this)" value="<s:property value="userID" />"> </td>

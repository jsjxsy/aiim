...
<html>
  <head>
	...
	<script type="text/javascript" src="dwr/interface/YJJSAction.js"></script>
...
//初始化///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function init() {  
         ...
         if(obj.deptType == "YWZDS"){
             $("checkPass").value = "通      过";
         }else if(obj.deptType == "DAGLS"){
             $("checkPass").value = "归      档";
         }
 	}
...
//单击事件////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//接收审核通过
    function checkPass(){
        YJJSAction.paperCheckPassDWR(archivesTypeID,NBXHS[0],obj.batNo,obj.deptType,paperCheckPassDWRCallBack); 
    }
    function paperCheckPassDWRCallBack(data){
       if(data == 1){
    	   window.returnValue=1;
           window.close();
       }
	...
	//接受审核打回
	function checkOut(){
		var returnValue = window.showModalDialog("/aiim/DAGL/SHDH.jsp","newwindow","dialogWidth=500px;dialogHeight:240px;center=yes;help=no;resizable=no;status=no;scroll=no;");
		if(returnValue != null){
			YJJSAction.paperCheckBackDWR(archivesTypeID,NBXHS[0],returnValue,obj.batNo,obj.deptType,checkBackCallBack); 
		}
    }
    function checkBackCallBack(data){
    	if(data == 1){
     	   window.returnValue=1;
           window.close();
        }
...
<body onload="init()" style="margin:0px; background-color:#f9f9f9; text-align: center;">
	...
	<fieldset style="height:400px;width: 99%;font-size:12px;margin-top: 5px;">
   ...
   <table align="center" width="100%">
      <tr>
         <td align="center"><input type="button" value="通      过" id="checkPass" class="button" onclick="checkPass()"/>	<input type="button"  value="打      回" class="button" onclick="checkOut()" style="margin-left: 15px;"/></td>
      </tr>
  </table> 	
</body>
</html>

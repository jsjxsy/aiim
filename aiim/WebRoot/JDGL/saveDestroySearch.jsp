...
List results = (List)request.getAttribute("appraisalKeepDestroyDetails");
%>
...
<html>
  <head>
	...
	//////////////////单击事件////////////////////////
		...
		//显示详情
		 function showDetail(idObj){
			 var url = "archiveAppraisalAction_saveDestroySearchDetail.action?appraisalDetailId="+idObj;
			 showWinModalDialog(url ,'500px' ,'300px');
		 }
	</script>
</head>
  
<s:form id="conditionForm" name="conditionForm" action="archiveAppraisalAction_saveDestroySearch" namespace="/JDGL">
			...
			<tr>
				<td>
					...
					<div style="margin-right:2px; color:blue; display:block; width:300px; float:right; margin-top:5px;" align="right">
						<font style="font-size:12px;font-weight:bold;">当前位置：</font>鉴定登记信息&nbsp;&gt;&gt;&nbsp;存毁鉴定
					</div>					
				</td>
			</tr>
			<tr>
			...
			<tr id="tr_title" style="display: none;">
				<td>
					<table class="tabletop" width="100%">
						<tr>
			                <td class="tableTitle">存毁鉴定登记信息</td>
			...
			<tr id="tr_showTable" style="display: none;">
				<td>
					<table id="showTable" width="100%" style="margin:0px; border: #104da6 1px solid;" cellspacing="1px" cellpadding="0px" >						
						...
						<tbody id="checkList">
							<s:iterator value="#request.appraisalKeepDestroyDetails" var="entity" status="stau">
								<s:if test="#stau.count%2==0">
									<tr  bgcolor="#eef5ff" id="row<s:property value="#stau.count"/>" onclick="clickRow(this)">
								</s:if>
								<s:else>
									<tr  bgcolor="#e0edff" id="row<s:property value="#stau.count"/>" onclick="clickRow(this)">
								</s:else>
									...
									<td>
										<s:if test="#entity.AppraisalDeletedFlag == 1">
											销毁
										</s:if>
										<s:else>
											<s:property value="#entity.retentionPeriodName"/>
										</s:else>
									</td>
									<td><s:property value="#entity.oldRetentionPeriodName"/></td>
									<td><fmt:formatDate value="${entity.appraisalDate}" type="date"/></td>
									<td><a href="javascript:showDetail('<s:property value="#entity.iD"/>')">详情</a></td>
								</tr>
							</s:iterator>
						</tbody>
					</table>

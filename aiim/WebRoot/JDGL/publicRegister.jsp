  ...
  <head>
	...
	//////////////////单击事件////////////////////////
		...
		function toRegister() {
			...
			if(flag) {
				$('form').attr('action','JDGL/archiveAppraisalAction_publicRegister.action');
				$('form').submit();
			} 
			return;
		}
		...
		var g_NBXH;
		//取消开放鉴定
		function cancelPublic(NBXH) {
			var archivesTypeId = $('#nodeId').val();
			g_NBXH = NBXH;
			ArchiveAppraisalDWR.cancelPublicOpenRegister(archivesTypeId, NBXH, 1, cancelPublicOpenRegisterBak);
...
<body>
<s:form id="conditionForm" name="conditionForm" action="archiveAppraisalAction_searchBeforePublicRegister" namespace="/JDGL">
					   	    ...
					   	    <%
					   	     ...
					   	     if(archivesInfos != null){
					   	        for(int i =0;i<archivesInfos.size();i++){
					   	    	    ...
					   	    	    if(i%2 == 0){    		
					   	    		    out.print("<tr bgcolor=\"#eef5ff\" id=\""+archivesInfo.getNBXH()+"\"  onclick=\"clickRow(this)\" >");   	    		
					   	    	    }else{
					   	    		    out.print("<tr bgcolor=\"#e0edff\"  id=\""+archivesInfo.getNBXH()+"\" onclick=\"clickRow(this)\" >");
					   	    	    }
					   	    	    ...
					   	    	    //判断是否进行开放鉴定
					   	    	    if(archivesInfo.getPublicFlag()) {	//取消开放鉴定
					   	    	    	out.print("<td align='center'><a href=\"javascript:cancelPublic('"+archivesInfo.getNBXH()+"')\">取消</a></td>");
					   	    	    } else {	//进行开放鉴定
					   	    	   		//开放	单选按钮
					...
					<fieldset id="publicField">
						<legend>开放鉴定信息登记</legend>
							<table cellpadding="0" cellspacing="0" style="width:100%;font-size: 12px;">
								...
								<tr style="height:24px;">
									<td>
									开放依据<textarea id="AppraisalReason" name="AppraisalReason" style="width:486px;margin-left:13px;"></textarea>

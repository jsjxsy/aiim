...
function receiveCondition() {
	var url = "workProcedureMonitorAction_archivesinfoManagerRevceiverCondition.action";
	showWinModalDialog(url, '580px', '400px');
}
</script>
  </head>
  <body>
  <s:form action="workProcedureMonitorAction_manage" namespace="/JXGL">
  	<s:hidden name="nodeId" value="13" />
  	<table width="100%" style="margin:0px;font-size: 12px;" cellspacing="0" cellpadding="0">
			<tr>
				<td>
		          <div style="margin-right:1px; display:block; float:right;margin-top:0px; color:blue;width: 570px;">
						<font style="font-size:13px;font-weight:bold;">当前位置:绩效管理&gt;&gt;工作过程监测&gt;&gt;入库工作量&nbsp;&nbsp;当前时间：<span id="currentTime"></span></font>
				  </div>
				</td>
			</tr>
			<tr>
				<td>
				  	&nbsp;监测时间截止到<s:textfield name="endTime" id="endTime" theme="simple"/><image style="margin-right:20px;" src="images/dropdownTime.gif" onclick="PopUpCalendar('endTime',true)"  />
				  	<input type="button" onclick="queryCondition()" value="查看" />&nbsp;&nbsp;
				  	<input type="button" onclick="receiveCondition()" value="档案接收情况" />
				</td>
			</tr>
			<tr>
				<td>

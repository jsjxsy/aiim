...
<html>
  <head>
	...
	<script type="text/javascript" src="dwr/interface/studentFileManageDWR.js"></script>
	...
	//打印快递单
	 function click_print(){
		 //调用DWR查找快递单对象集合
		 var checkBoxs = $(":checked[name='ids']");
		 var array = new Array();
		 for(var i=0;i<checkBoxs.length;i++){
			 array[i]=checkBoxs[i].value;
		 }
		 studentFileManageDWR.getEMSInfos(array,getEMSInfosCallBack);	
	 }
	 function getEMSInfosCallBack(data){
		 var PrintTool = document.getElementById("printPostReceipt");
		 var ret;
	     var errinfo;
		 for(var i=0;i<data.length;i++){	
			// var PrintTool1 = document.printPostReceipt;
		     PrintTool.SJJ = data[i].acceptanceOffice;
		     // data[i].acceptancePersonName;
		     PrintTool.JJRXM = data[i].fromPersonName;
		     PrintTool.JJRDH = data[i].fromPersonPhone;
		     PrintTool.JJRDWMC = data[i].fromCompanyName;
		     PrintTool.JJRDZ = data[i].fromCompanyAddr;
		     PrintTool.JJRYZBM = data[i].fromPostCode;
		     PrintTool.NJPM = data[i].contents;
		     PrintTool.SL = data[i].contentTol;
		     PrintTool.JJRQM = data[i].remailPersonName;
		    
		     PrintTool.SJRXM = data[i].receivePersonName;
		     PrintTool.SJRDH = data[i].receivePersonPhone;
		     PrintTool.SJRDWMC = data[i].receiveCompanyName;
		     PrintTool.SJRDZ = data[i].receiveCompanyAddr;
		     PrintTool.SJRCS = data[i].receiveCityName;
		     PrintTool.SJRYZBM = data[i].receivePostCode;
		    
		     ret = PrintTool.PrintEMS(errinfo);
		     if(ret){
		        //alert("ok!");
		     }else{
		        alert("错误：" +errinfo);
...
<object id="printPostReceipt" name="printPostReceipt" classid="clsid:DE020802-1943-4B78-9940-C01756B63698" codeBase="<%=basePath%>ActiveX/PrintPostReceipt.CAB#version=1,0,0,0" style="width: 0px;height: 0px; display: none;"></object>
	...
	<tr>
		<td><!-- 
			<input type="image" src="images/print.gif" style="margin-left:3px;" onmouseover="changeImage(this,'print2.gif')" onmouseout="changeImage(this,'print.gif')" onclick="alert('打印中...')"/>
			 -->
			 <input type="button" value="打印快递单" onclick="click_print()">
...
<form name="conditionForm" action="XSDAGL/studentFileManageAction_findMoveOutInfo.action" method="post" style="margin: 0;padding: 0;">	
	...
	<input type="hidden" id="moveOutWay" name="moveOutWay" value="1">
</form>		
<table width="100%" style="margin:0px; font-size:12px;" cellspacing="0" cellpadding="0">

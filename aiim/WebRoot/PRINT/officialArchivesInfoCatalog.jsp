...
List<OfficialArchivesInfo> officialArchivesInfos = (List<OfficialArchivesInfo>)request.getAttribute("officialArchivesInfos");
  ...
  <table id="tablePrint" class="tablePrint" style="font-size: 12px;width: 600px;" cellspacing="1px" cellpadding="0px" border="1">
		...
		<tbody id="printPage">
			...
			//注意有事件类型的参数
	   	     ...
	   	     if(officialArchivesInfos != null && officialArchivesInfos.size()>=1){
	   	        for(int i =0;i<officialArchivesInfos.size();i++){
	   	        	officialArchivesInfo  = officialArchivesInfos.get(i);
					out.print("<tr>");
	   	    	    out.print("<td>"+(i+1)+"</td>");
	   	    	    for(ArchivesTypeDataItem dataItem : viewDataItems){
	   	    	    	FieldValue fieldValue = officialArchivesInfo.getRowFieldsValues().get(dataItem.getColumnName());
	   	    		   if(fieldValue == null){
	   	    			  out.print("<td>&nbsp;</td>");
	   	    		   }else{

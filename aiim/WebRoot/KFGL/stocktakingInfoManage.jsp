...
//打开库房盘点页面
function openStocktaking(stocktakingId){
	alert(stocktakingId);
	var returnValue = window.showModalDialog("<%=basePath%>KFGL/storeroomManageAction_stocktakingBefore.action?stocktakingId="+stocktakingId,"newwindow","dialogWidth=550px;dialogHeight:120px;top=0;left=0;status:no; toolbar=no;menubar=no;scroll:0;location:yes; resizable=no;location=n");
	if(returnValue==true){
...
//查看库房盘点结果
function viewStocktakingReports(stocktakingId){
	alert(stocktakingId);
	var returnValue = window.showModalDialog("<%=basePath%>KFGL/storeroomManageAction_findStocktakingReports.action?stocktakingId="+stocktakingId,"newwindow","dialogWidth=950px;dialogHeight:700px;top=0;left=0;status:no; toolbar=no;menubar=no;scroll:0;location:yes; resizable=no;location=n");
	if(returnValue==true){
		window.location.reload();
	}
}
...
function showAdd()
{
	var returnValue = window.showModalDialog("<%=basePath%>KFGL/storeroomManageAction_addStocktakingBefore.action","newwindow","dialogWidth=300px;dialogHeight:80px;top=0;left=0;status:no; toolbar=no;menubar=no;scroll:0;location:yes; resizable=no;location=n");
	if(returnValue==true){
		window.location.reload();
	}
}
...
function deleteSelected(){
  ...
  <input type="hidden" name="preSelectRow" id="preSelectRow" />
	       ...
	       <tr>
			   ...
			   <td>
				   <div id="location" style="margin-right: 2px; width: 280px; display: inline-block; float: right; margin-top: 0px; color: blue; text-align: right;">
					  <font style="font-size: 12px;"><b>当前位置：</b>库房管理&nbsp;&gt;&gt;&nbsp;&nbsp;库房盘点</font>
				   </div>
			   </td>
	       </tr>
	     </table>
			 
		<form name="conditionForm" action="KFGL/storeroomManageAction_findStocktakingInfos.action" method="post" style="margin: 0;padding: 0;" >
		...
		<table width="100%" style="margin:0px; font-size: 12px;" cellspacing="0" cellpadding="0">
			...
			<form name="deleteForm" action="KFGL/storeroomManageAction_deleteStocktakings.action" method="post" style="margin: 0;padding: 0;">
				<table id="showTable" width="100%" style="margin:0px; border: #104da6 1px solid;" cellspacing="1px" cellpadding="0px" >						
					...
					<tbody>
						<s:iterator value="#request.stocktakingInfos" status="rowStatus">
							...
							<tr bgcolor="${pageScope.color}" id="row<s:property value="ID"/><s:property value="ID"/>" onclick="clickRow1(this)" ondblclick="showAlter(<s:property value="ID"/>)">
								<td align="center" width="30px;"><input type="checkbox" name="stocktakings" value="<s:property value="ID"/>" id="<s:property value="ID"/>" /></td>
								<td align="center" width="30px;"><s:property value="#rowStatus.index+1"/></td>
								<td><s:date name="stocktakingDate" format="yyyy"/>年<s:property value="storeroomName"/>库房盘点</td>
								<td><s:property value="storeroomName"/></td>
								<td><s:date name="stocktakingDate" format="yyyy-MM-dd"/></td>
								<td><s:if test="stocktakedFlag==true">已盘点</s:if><s:else>未盘点</s:else></td>
								<td><a href="javascript:openStocktaking(<s:property value="ID"/>)">盘点</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:viewStocktakingReports(<s:property value="ID"/>)">查看</a></td>
							</tr>
						</s:iterator>
					</tbody>
				</table>
